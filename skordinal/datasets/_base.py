...
def _resolve_train_masks(csv_path, X, y, ids, random_state):
    ...
    # Validate ids up front so every resolution path below rejects them alike
    for rid in ids:
...
def load_partitions(
    ...
    *,
    ...
    resamples : int or list of int, default=30
        ...
        list, those ids are used directly and must be unique, non-negative
        integers. Must be >= 2 when masks are generated by
        cross-validation.
    ...
    ValueError
        When ``resamples < 2`` and CV masks must be generated, when the
        resample ids are negative or duplicated, when a mask does not hold
        boolean or integer values, when a mask does not have one entry per
        sample, when a mask is degenerate (all True or all False), or when
        the target column does not hold integer class labels.
    TypeError
        When a list of resample ids holds a non-integer.
    KeyError
