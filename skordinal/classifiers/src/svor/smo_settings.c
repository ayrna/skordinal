...
smo_Settings * Create_smo_Settings_Python ( def_Settings * settings ) 
{
	...
	if (NULL != (psetting = (smo_Settings *)(malloc(sizeof(smo_Settings))))) 	
	{
		...
		/*/ Create_Alphas below branches on this, so set it first*/
		psetting->constraints = CONSTRAINTS ;
		...
		if (ORDINAL == settings->pairs.datatype)
		{
			...
			if (EXPLICIT_CONSTRAINTS == psetting->constraints)
			{
				/*/ multipliers of the explicit b_j <= b_j+1 constraints*/
