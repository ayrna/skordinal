...
/*******************************************************************************\
	...
	BOOL ordinal_takestep_implicit ( Alphas *, Alphas *, unsigned int, smo_Settings * )

	the implicit-constraint counterpart of ordinal_takestep. Every sample takes
	part in every threshold, so the four cases are selected by inequality on the
	target rather than equality, and the box is derived from the two signs s1
	and s2 instead of from alpha_up and alpha_dw. Ported from the
	implicit-constraint SMO of Chu and Keerthi (2007), Check_Alphas tail included.
