...
with open(archivo_salida, 'w', encoding='utf-8') as f_salida:
    ...
    for carpeta in carpetas:
        ...
        if not os.path.exists(carpeta):
            print(f"⚠️ Advertencia: La carpeta '{carpeta}' no existe o no se encuentra. Se omitirá.")
            continue
            ...
            if os.path.isfile(ruta_archivo):
                try:
                    ...
                    with open(ruta_archivo, 'r', encoding='utf-8') as f_entrada:
                        ...
                        f_salida.write(f"\n{'='*60}\n")
                        f_salida.write(f"CARPETA: {carpeta} | ARCHIVO: {nombre_archivo}\n")
                        f_salida.write(f"{'='*60}\n\n")
                    ...
                    print(f"❌ Error de codificación: No se pudo leer '{ruta_archivo}'. Asegúrate de que sea un archivo de texto.")
                except Exception as e:
                    print(f"❌ Error inesperado al leer '{ruta_archivo}': {e}")

print(f"✅ Proceso completado. El archivo '{archivo_salida}' ha sido generado con éxito.")